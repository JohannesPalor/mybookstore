...
namespace MyBookstore.Controllers
{
    public class PublishersController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            using (SqlConnection con = new SqlConnection(Yelle.GetCon()))
            {
                con.Open();
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                    {
                        using (DataTable dt = new DataTable())
                        {
                            ...
                            foreach(DataRow row in dt.Rows)
                            {
                                ...
                                publisher.PubName = row["pubName"].ToString();
                                list.Add(publisher);
                            }
                        }
                    }
        ...
        public ActionResult Create(PublishersModels publisher)
        {
            ...
            using (SqlConnection con = new SqlConnection(Yelle.GetCon()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@pubName", publisher.PubName);
                    cmd.ExecuteNonQuery();
                    return RedirectToAction("Index");
                }
            }
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                publisher.PubName = dr["pubName"].ToString();
                            }
